...
Dispositions: 20 fixed, 0 recorded, 1 ruled not a defect, 1 still open in part (D102's wording, which goes with the summary rewrite after the dose-fixed run).
...
- **D102** — fixed in part; still open on the wording — `7f5b39e` added `noise_by_condition` and answering cells: `summary.ipynb` L452-475 print HELD / DID NOT HOLD for hypothesis 3 (Spearman of split-half r against per-condition variance) and hypothesis 4 (responder against non-responder variance) from `rung0_noise_by_condition.csv`. Hypothesis 3 at L152 still reads "with lower correlations", truncating `design.md:299`'s "over either all genes or the differentially expressed genes", and the answering cell tests the all-gene `r` only.
